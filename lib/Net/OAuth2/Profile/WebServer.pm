...
sub get_access_token {
	...
	my %prm  = @_;
	my $dta  = {};
	if( my $code = delete $prm{code} ) { #assume this is the first call
		my $request;
		if ($self->client->access_token_method eq 'POST') {
			$request = POST($self->client->access_token_url(), {$self->access_token_params($code, %prm)});
		} else {
			$request = HTTP::Request->new(
				$self->client->access_token_method => $self->client->access_token_url(
					$self->access_token_params($code, %prm)
				)
			);
		}
